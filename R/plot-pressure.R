...
#'
#' @param wheel Indicates which wheel. Value should be "Front" or "Rear".
#' @param psi Inflation pressure for the wheel.
#'
#' @return
#' @export
#'
dual_pressure_point <- function(wheel = c("Front", "Rear"), psi) {
  return(sprintf('%s\n%d psi\n%.1f bar', wheel, psi, psi_to_bar(psi)))
}

base_pressure_plot <- ggplot(inflation_data,
...
#                    theme(legend.position = c(0.08, 0.735), legend.justification = c(0, 1)) +
                    ...
                    annotate("rect", xmin = 66, xmax= 160, ymin = 20, ymax = 105, alpha = 0.1, fill = safe_pressure_color) +
                    annotate("text",
